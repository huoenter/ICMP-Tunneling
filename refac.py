	...
	def run(self):
		...
		while True:
			...
			if not reverseReady:
				...
				T2I_pipe(self.sink, self.source, socket.inet_ntoa(address)).start()
				time.sleep(1)
				reverseReady = True
				first = True
			if code == DRINKER:
			...
			if DEBUG and not first: 
				print '%s: ICMP from %s RELAYto %s -> %s' % \
					(name, socket.inet_ntoa(address), self.sink.getsockname(), self.sink.getpeername())
				first = False
...
class Pipe_T2I(threading.Thread):
	...
	def run(self):
		...
		while True:
			...
			if not buf: break
			data = wrap(buf, DRINKER)
			self.sink.sendto(data, (self.ICMP_ADDRESS, 22))
			try:
				if DEBUG: print '%s: %s -> %s RELAYto ICMP -> %s' % \
						(name, self.source.getpeername(), self.getsockname(), self.ICMP_ADDRESS)
			except: pass
